...
fn change_collection_issuer_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// ALICE can't change ownership to BOB because BOB had not accept
		assert_noop!(
			RMRKCore::change_collection_issuer(Origin::signed(ALICE), 0, BOB),
			UNQ::Error::<Test>::Unaccepted
		);
		assert_ok!(UNQ::Pallet::<Test>::set_accept_ownership(Origin::signed(BOB), Some(0)));
