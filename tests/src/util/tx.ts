...
export async function createCollection(
  ...
  expect(collectionResult.success, "Error: unable to create a collection").to.be
    .true;
  ...
  expect(collection.metadata.toUtf8()).to.be.equal(
    metadata,
    "Error: Invalid NFT collection metadata"
  );
  expect(collection.max.isSome).to.be.equal(
    max !== null,
    "Error: Invalid NFT collection max"
  );

  if (collection.max.isSome) {
    expect(collection.max.unwrap().toNumber()).to.be.equal(
      max,
      "Error: Invalid NFT collection max"
    );
  }
  expect(collection.symbol.toUtf8()).to.be.equal(
    symbol,
    "Error: Invalid NFT collection's symbol"
  );
  expect(collection.nftsCount.toNumber()).to.be.equal(
    0,
    "Error: NFT collection shoudn't have any tokens"
  );
  expect(collection.issuer.toString()).to.be.equal(
    issuer.address,
    "Error: Invalid NFT collection issuer"
  );
...
export async function changeIssuer(
  ...
  expect(isTxResultSuccess(events), "Error: Unable to accept ownership").to.be
    .true;
  ...
  expect(
    changeIssuerResult.success,
    "Error: Unable to change NFT collection issuer"
  ).to.be.true;
  expect(
    changeIssuerResult.successData!,
    "Error: Invalid collection id after changing the issuer"
  ).to.be.eq(collectionId);

  await getCollection(api, collectionId).then((collectionOption) => {
    const collection = collectionOption.unwrap();
    expect(collection.issuer.toString()).to.be.deep.eq(
      bob.address,
      "Error: Invalid NFT collection issuer"
    );
  });
...
export async function deleteCollection(
  ...
  expect(collectionTxResult.success, "Error: Unable to delete NFT collection")
    .to.be.true;

  const collection = await getCollection(api, parseInt(collectionId, 10));
  expect(collection.isEmpty, "Error: NFT collection should be deleted").to.be
    .true;
...
export async function setNftProperty(
  ...
  const tx = api.tx.rmrkCore.setProperty(collectionId, nftIdOpt, key, value);
  const events = await executeTransaction(api, issuer, tx);
  ...
  expect(
    ...
    "Error: Invalid collection ID " + eventDescription
  ).to.be.equal(collectionId);

  expect(
    eventData.nftId.eq(nftIdOpt),
    "Error: Invalid NFT ID " + eventDescription
  ).to.be.true;

  expect(
    eventData.key.eq(key),
    "Error: Invalid property key " + eventDescription
  ).to.be.true;

  expect(
    eventData.value.eq(value),
    "Error: Invalid property value " + eventDescription
  ).to.be.true;
...
export async function mintNft(
  ...
  const ss58Format = api.registry.getChainProperties()!.toJSON().ss58Format;
  const issuer = privateKey(issuerUri, Number(ss58Format));
  const owner = ownerUri
    ? privateKey(ownerUri, Number(ss58Format)).address
    : null;
  const recipient = recipientUri
    ? privateKey(recipientUri, Number(ss58Format)).address
    : null;
  ...
  const oldCollectionNftsCount = collectionOpt.unwrap().nftsCount.toNumber();

  expect(
    newCollectionNftsCount,
    "Error: NFTs count should increase"
  ...
  expect(nft.owner.isAccountId, "Error: NFT owner should be some user").to.be
    .true;
  expect(nft.owner.asAccountId.toString()).to.be.equal(
    actualOwnerAddress,
    "Error: Invalid NFT owner"
  );

  const isOwnedInUniques = await isNftOwnedBy(
    api,
    actualOwnerUri,
    collectionId,
    nftId
  ...
  expect(
    isOwnedInUniques,
    `Error: created NFT is not actually owned by ${ownerUri}`
  ).to.be.true;

  if (recipient === null && royalty === null) {
    expect(nft.royalty.isNone, "Error: Invalid NFT recipient").to.be.true;
  } else {
    expect(nft.royalty.isSome, "Error: NFT royalty not found").to.be.true;

    const nftRoyalty = nft.royalty.unwrap();
    expect(nftRoyalty.recipient.eq(recipient), "Error: Invalid NFT recipient")
      .to.be.true;

    expect(nftRoyalty.amount.eq(royalty), "Error: Invalid NFT royalty").to.be
      .true;
  }

  expect(nft.metadata.toUtf8()).to.be.equal(
    metadata,
    ...
    for (let i = 0; i < resources.length; i++) {
      ...
      for (let j = 0; j < nftResources.length && !successFindingResource; j++) {
        ...
        if (
          ...
          if (
            (typedResource.parts != undefined &&
              typedResource.parts.toString() !=
                typedNftResource.parts.toString()) ||
            (typedResource.base != typedNftResource.base &&
              typedResource.base != undefined)
          ) {
            continue;
          }
        } else if (
        ...
        if (typedResource.metadata != typedNftResource.metadata) {
          continue;
        }

        // do final checks since this is now established to be the resource we seek
        expect(nftResourceData.pending, `Error: Resource #${i} is pending`).to
          .be.false;
        expect(
          nftResourceData.pendingRemoval,
          `Error: Resource #${i} is pending removal`
        ).to.be.false;
      ...
      expect(
        successFindingResource,
        `Error: Couldn't find resource #${i}'s counterpart among the returned`
      ).to.be.true;
    }
  }
...
export async function sendNft(
  ...
  expect(
    sendData.dstOwner.eq(newOwnerObj),
    "Error: Invalid target user (from event data)"
  ).to.be.true;

  expect(sendData.collectionId).to.be.equal(
    collectionId,
    "Error: Invalid collection ID (from event data)"
  );

  expect(sendData.nftId).to.be.equal(
    nftId,
    "Error: Invalid NFT ID (from event data)"
  );

  expect(
    nftBeforeSendingOpt.isSome,
    "Error: Unable to fetch NFT before sending"
  ).to.be.true;
  ...
  expect(nftAfterSendingOpt.isSome, "Error: Unable to fetch NFT after sending")
    .to.be.true;

  const nftAfterSending = nftAfterSendingOpt.unwrap();

  const isOwnedByNewOwner = await isNftOwnedBy(
    api,
    newOwner,
    collectionId,
    nftId
  ...
  expect(
    nftAfterSending.royalty.eq(nftBeforeSending.royalty),
    "Error: Invalid NFT royalty after sending"
  ).to.be.true;

  expect(
    nftAfterSending.metadata.eq(nftBeforeSending.metadata),
    "Error: Invalid NFT metadata after sending"
  ).to.be.true;

  expect(
    nftAfterSending.equipped.eq(nftBeforeSending.equipped),
    "Error: Invalid NFT equipped status after sending"
  ).to.be.true;

  expect(
    nftAfterSending.pending.eq(isPending),
    "Error: Invalid NFT pending state"
  ).to.be.true;
}
...
export async function acceptNft(
  ...
  expect(
    acceptData.recipient.eq(newOwnerObj),
    "Error: Invalid NFT recipient (from event data)"
  ).to.be.true;

  expect(acceptData.collectionId).to.be.equal(
    collectionId,
    "Error: Invalid collection ID (from event data)"
  );

  expect(acceptData.nftId).to.be.equal(
    nftId,
    "Error: Invalid NFT ID (from event data)"
  );
  ...
  expect(
    isOwnedInUniques,
    `Error: created NFT is not actually owned by ${newOwner.toString()}`
  ).to.be.true;
}
...
export async function rejectNft(
  ...
  const events = await executeTransaction(api, issuer, tx);
  const rejectResult = extractRmrkCoreTxResult(
    events,
    "NFTRejected",
    (data) => {
      return {
  ...
  expect(rejectData.collectionId).to.be.equal(
    collectionId,
    "Error: Invalid collection ID (from event data)"
  );

  expect(rejectData.nftId).to.be.equal(
    nftId,
    "Error: Invalid NFT ID (from event data)"
  );
  ...
  expect(nftAfter.isNone, "Error: NFT should be burned after reject").to.be
    .true;
}
...
export async function createBase(
  ...
  expect(baseResult.success, "Error: Unable to create Base").to.be.true;

  baseId = baseResult.successData!;
  const baseOptional = await getBase(api, baseId);

  expect(baseOptional.isSome, "Error: Unable to fetch created Base").to.be.true;

  const base = baseOptional.unwrap();
  const baseParts = await getParts(api, baseId);

  expect(base.issuer.toString()).to.be.equal(
    issuer.address,
    "Error: Invalid Base issuer"
  );
  expect(base.baseType.toUtf8()).to.be.equal(
    baseType,
    "Error: Invalid Base type"
  );
  expect(base.symbol.toUtf8()).to.be.equal(
    symbol,
    "Error: Invalid Base symbol"
  );
  expect(partTypes.eq(baseParts), "Error: Received invalid base parts").to.be
    .true;
...
export async function setResourcePriorities(
  ...
  expect(prioResult.success, "Error: Unable to set resource priorities").to.be
    .true;
  const eventData = prioResult.successData!;

  expect(eventData.collectionId).to.be.equal(
    collectionId,
    "Error: Invalid collection ID (set priorities event data)"
  );

  expect(eventData.nftId).to.be.equal(
    nftId,
    "Error: Invalid NFT ID (set priorities event data"
  );

  for (var i = 0; i < priorities.length; i++) {
    const resourceId = priorities[i];

    const fetchedPrio = await getResourcePriority(
      api,
      collectionId,
      nftId,
      resourceId
...
export async function setEquippableList(
  ...
  const ss58Format = api.registry.getChainProperties()!.toJSON().ss58Format;
  const issuer = privateKey(issuerUri, Number(ss58Format));
  const equippable = api.createType(
  ...
  if (fetchedEquippableList) {
    expect(fetchedEquippableList).to.be.deep.equal(
      equippableList,
      "Error: invalid equippable list was set"
    );
  }
}
...
export async function addTheme(
  ...
  const fetchedThemeOpt = await getTheme(
    api,
    baseId,
    theme.name.toUtf8(),
    null
  ...
  expect(theme.name.eq(fetchedTheme.name), "Error: Invalid theme name").to.be
    .true;
  ...
  for (var i = 0; i < theme.properties.length; i++) {
    ...
    const propertyFoundCount = fetchedTheme.properties.filter((fetchedProp) =>
      property.key.eq(fetchedProp.key)
    ).length;

    expect(
      propertyFoundCount > 1,
      `Error: Too many properties with key ${propertyKey} found`
    ).to.be.false;

    if (filterKeys) {
      const isFiltered =
        fetchedTheme.properties.find((fetchedProp) =>
          fetchedProp.key.eq(property.key)
        ) === undefined;

      if (isFiltered) {
        expect(
          propertyFoundCount === 0,
          `Error: Unexpected filtered key ${propertyKey}`
        ).to.be.true;
        continue;
      }
    }
    ...
    expect(
      propertyFoundCount === 1,
      `Error: The property with key ${propertyKey} is not found`
    ).to.be.true;
  }
}
...
export async function lockCollection(
  ...
  expect(
    ...
    "Error: Invalid collection was locked"
  ).to.be.eq(collectionId);
...
export async function setPropertyCollection(
  ...
  expect(propResult.success, "Error: Unable to set collection property").to.be
    .true;
  const eventData = propResult.successData!;
  ...
  expect(
    eventData.nftId.eq(null),
    "Error: Unexpected NFT ID " + eventDescription
  ).to.be.true;
  ...
  expect(await isCollectionPropertyExists(api, collectionId, key, value)).to.be
    .true;
}
...
export async function burnNft(
  ...
  expect(burnResult.successData!, "Error: Invalid NFT was burned").to.be.eq(
    nftId
  );
...
export async function acceptNftResource(
  ...
  const tx = api.tx.rmrkCore.acceptResource(collectionId, nftId, resourceId);
  ...
  expect(
    acceptResult.successData!.nftId,
    "Error: Invalid NFT ID while accepting a resource"
  ).to.be.eq(nftId);
  expect(
    acceptResult.successData!.resourceId,
    "Error: Invalid resource ID while accepting a resource"
  ).to.be.eq(resourceId);
...
export async function addNftBasicResource(
  ...
  expect(
    Object.prototype.hasOwnProperty.call(resourceJson, "Basic"),
    "Error: Expected basic resource type"
  ).to.be.true;

  const recvBasicRes = resourceJson["Basic"];

  expect(
    recvBasicRes.metadata,
    "Error: basic first resource metadata"
  ).to.be.eq(metadata);
...
export async function addNftComposableResource(
  ...
  const composableResource = api.createType(
  ...
  expect(
    Object.prototype.hasOwnProperty.call(resourceJson, "Composable"),
    "Error: Expected composable resource type"
  ).to.be.true;

  const recvComposableRes = resourceJson["Composable"];

  expect(
    recvComposableRes.parts.toString(),
    "Error: Invalid composable resource parts"
  ).to.be.eq(parts.toString());
  expect(
    recvComposableRes.base,
    "Error: Invalid composable resource base id"
  ).to.be.eq(baseId.toString());
  expect(
    recvComposableRes.metadata,
    "Error: Invalid composable resource metadata"
  ).to.be.eq(metadata);
...
export async function addNftSlotResource(
  ...
  expect(
    Object.prototype.hasOwnProperty.call(resourceJson, "Slot"),
    "Error: Expected slot resource type"
  ).to.be.true;

  const recvSlotRes = resourceJson["Slot"];

  expect(recvSlotRes.base, "Error: Invalid slot resource base id").to.be.eq(
    baseId.toString()
  );
  expect(recvSlotRes.slot, "Error: Invalid slot resource slot id").to.be.eq(
    slotId.toString()
  );
  expect(
    recvSlotRes.metadata,
    "Error: Invalid slot resource metadata"
  ).to.be.eq(metadata);
...
export async function equipNft(
  ...
  expect(
    equipResult.successData!.item_collection,
    "Error: Invalid item collection id"
  ).to.be.eq(item[0]);
  expect(
    equipResult.successData!.item_nft,
    "Error: Invalid item NFT id"
  ).to.be.eq(item[1]);
  expect(equipResult.successData!.base_id, "Error: Invalid base id").to.be.eq(
    base
  );
  expect(equipResult.successData!.slot_id, "Error: Invalid slot id").to.be.eq(
    slot
  );
}
...
export async function unequipNft(
  ...
  expect(
    unEquipResult.successData!.item_nft,
    "Error: Invalid item NFT id"
  ).to.be.eq(item[1]);
  expect(unEquipResult.successData!.base_id, "Error: Invalid base id").to.be.eq(
    base
  );
  expect(unEquipResult.successData!.slot_id, "Error: Invalid slot id").to.be.eq(
    slot
  );
}
...
export async function removeNftResource(
  ...
  expect(
    removeResult.successData!.nftId,
    "Error: Invalid NFT Id while removing a resource"
  ).to.be.eq(nftId);
  expect(
    removeResult.successData!.resourceId,
    "Error: Invalid resource Id while removing a resource"
  ).to.be.eq(resourceId);

  let afterDeleting = await findResourceById(
    api,
    collectionId,
    nftId,
    resourceId
...
export async function acceptResourceRemoval(
  ...
  const tx = api.tx.rmrkCore.acceptResourceRemoval(
    collectionId,
    nftId,
    resourceId
  );
